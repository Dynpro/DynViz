...
const renderList = arr => {
  if (arr && arr.length) {
    return arr.map((item, index) => {
      const value = item.value != null ? String(item.value) : 'N/A';
      return (
        <Box
          key={index}
          sx={{
  ...
  if (arr && arr.length) {
    return arr.map((item, index) => {
      ...
      return (
        ...
        >
          <Icon icon={item.icon} />
          <Typography sx={{ mx: 2, fontWeight: 600, color: 'text.secondary' }}>{item.property}:</Typography>
...
const AboutOverivew = ({ teams, about, contacts }) => {
  return (
    <Card>
      <CardContent>
        <Grid container spacing={6}>
