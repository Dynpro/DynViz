...
// session management by arman kham
  ...
  return (
    <Fragment>
      ...
      >
            ...
            >
              ...
              {item.children && item.children.length ? (
                <Box
                  sx={{
                    ml: 2,
                    transform: `rotate(${groupActive.includes(item.title) ? 180 : 0}deg)`,
                    transition: 'transform .25s ease-in-out'
                  }}
                >
        ...
        {item.children && item.children.length ? (
          <Collapse in={groupActive.includes(item.title)}>
            <VerticalNavItems
              items={item.children}
              settings={settings}
              navHover={navHover}
